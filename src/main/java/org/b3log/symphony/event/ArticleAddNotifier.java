...
public class ArticleAddNotifier extends AbstractEventListener<JSONObject> {
    ...
    public void action(final Event<JSONObject> event) {
        ...
        try {
            ...
            atNotification(hasAtUserPerm, originalArticle, articleAuthorName, articleId, atedUserIds);

            final String tags = originalArticle.optString(Article.ARTICLE_TAGS);

            // 'following - user' Notification
            followersNotification(data, originalArticle, tags, articleAuthorId, atedUserIds, articleId);

            final String articleTitle = Escapes.escapeHTML(originalArticle.optString(Article.ARTICLE_TITLE));

            // 'Broadcast' Notification
            broadcastNotification(originalArticle, articleAuthorId, articleId);

            systemAnnouncementNotification(tags, articleId, articleTitle);
        } catch (final Exception e) {
            LOGGER.log(Level.ERROR, "Sends the article add notification failed", e);
        }
    }

    private void atNotification(boolean hasAtUserPerm, JSONObject originalArticle, String articleAuthorName, String articleId, Set<String> atedUserIds) throws ServiceException {
        if (hasAtUserPerm) {
            // 'At' Notification
    ...
    private void systemAnnouncementNotification(String tags, String articleId, String articleTitle) throws ServiceException {
    ...
    private void broadcastNotification(JSONObject originalArticle, String articleAuthorId, String articleId) throws ServiceException {
    ...
    private void followersNotification(JSONObject data, JSONObject originalArticle, String tags, String articleAuthorId, Set<String> atedUserIds, String articleId) throws ServiceException {
